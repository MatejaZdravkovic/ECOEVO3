...
class Community():
    ...
    def run(self, T, dt=None, reorder_types_by_phylogeny=True, integration_method='default', max_time_step=np.inf, print_events=None, progress_callback=None):

        self.print_events = print_events if print_events is not None else self.print_events

        # VALIDATE AND CONVERT dt
        if dt is None:
            dt = T / 1000.0  # Default: 1000 steps
            print(f"[Community.run] dt not specified, using default dt={dt:.2e}", flush=True)
        else:
            dt = float(dt)   # Convert to float
            if dt <= 0:
                raise ValueError(f"[Community.run] dt must be > 0, got dt={dt}")

        print(f"[Community.run] Starting: T={T}, dt={dt:.3e}, initial t={self.t:.3e}", flush=True)

        t_start   = self.t
        t_end     = self.t + T
        t_elapsed = 0

        # Track epoch counter for logging
        epoch_counter = 0

        self._t_series.expand_alloc((self._t_series.alloc[0], self._t_series.alloc[1] + int(T / dt)))
        self._N_series.expand_alloc((self._N_series.alloc[0], self._N_series.alloc[1] + int(T / dt)))
        self._R_series.expand_alloc((self._R_series.alloc[0], self._R_series.alloc[1] + int(T / dt)))
        ...
        while (t_elapsed < T):
            ...
            # Safe construction: ensure all points strictly within t_span
            t0, t1 = float(t_span[0]), float(t_span[1])
            dt_float = float(dt)

            # Generate points from t0 up to (but not past) t1
            t_eval = np.arange(t0, t1, dt_float, dtype=float)

            # Include endpoint if not already covered
            if t_eval.size == 0 or (t1 - t_eval[-1]) > 1e-12:
                t_eval = np.append(t_eval, t1)
            else:
                # Clamp last point to t1 if extremely close
                t_eval[-1] = t1

            # Final clamp: remove any points outside [t0, t1]
            t_eval = t_eval[(t_eval >= t0 - 1e-12) & (t_eval <= t1 + 1e-12)]
            t_eval[0] = t0
            t_eval[-1] = t1

            # DIAGNOSTIC: Verify t_eval is within t_span
            t_eval_min, t_eval_max = t_eval.min(), t_eval.max()
            tol = 1e-10
            if t_eval_min < t_span[0] - tol or t_eval_max > t_span[1] + tol:
                error_msg = (f"[Community.run] FATAL: t_eval not within t_span! "
                             f"t_span=[{t_span[0]:.15e}, {t_span[1]:.15e}], "
                             f"t_eval=[{t_eval_min:.15e}, {t_eval_max:.15e}], "
                             f"violations: min={t_eval_min - t_span[0]:.3e}, max={t_eval_max - t_span[1]:.3e}")
            ...
            # Store epoch start time for event guard
            self._epoch_t0 = self.t

            # Draw a random propensity threshold for triggering the next Gillespie event:
            # Add epsilon floor to prevent numerically fatal edge case where threshold ≈ 0
            self.threshold_event_propensity = max(np.random.exponential(1.0), 1e-12)
            ...
            #------------------------------
            # DIAGNOSTIC LOGGING
            print(f"[Community.run] solve_ivp: t_span=({t_span[0]:.6e}, {t_span[1]:.6e}), "
                  f"dt={dt:.3e}, t_eval=[{t_eval[0]:.6e}, {t_eval[-1]:.6e}], len={len(t_eval)}",
                  flush=True)
            ...
            # DIAGNOSTIC LOGGING
            epoch_counter += 1
            print(f"[Community.run] Epoch {epoch_counter}: sol.t[0]={sol.t[0]:.3e}, "
                  f"sol.t[-1]={sol.t[-1]:.3e}, len(sol.t)={len(sol.t)}, status={sol.status}",
                  flush=True)

            # EVENT DIAGNOSTICS
            if sol.status == 1:
                print(f"[Community.run]   EVENT OCCURRED: sol.message='{sol.message}'", flush=True)
                print(f"[Community.run]   t_events[0]={sol.t_events[0] if len(sol.t_events) > 0 and len(sol.t_events[0]) > 0 else 'none'}", flush=True)
                if len(sol.y_events) > 0 and len(sol.y_events[0]) > 0:
                    cumulative_prop = sol.y_events[0][0][-1] if len(sol.y_events[0][0].shape) > 0 else sol.y_events[0][0]
                    print(f"[Community.run]   Threshold={self.threshold_event_propensity:.6e}, "
                          f"Cumulative propensity at event={cumulative_prop:.6e}",
                          flush=True)

            # NEW: Handle events that occurred with minimal integration
            # If event fired but solver returned only initial point, process the event immediately
            if sol.status == 1 and (len(sol.t) < 2 or sol.t[-1] == t_span[0]):
                print(f"[Community.run]   EVENT with minimal integration detected", flush=True)

                # Extract event time
                if len(sol.t_events) > 0 and len(sol.t_events[0]) > 0:
                    t_event = sol.t_events[0][0]
                    print(f"[Community.run]   Processing event at t_event={t_event:.6e}", flush=True)

                    # Advance simulation time to event occurrence by adding to time series
                    self._t_series.add(np.array([t_event]), axis=1)

                    # Process the lineage establishment event
                    self.handle_lineage_establishment()

                    print(f"[Community.run]   Event processed: t={self.t:.6e}, "
                          f"types={self.type_set.num_types}, "
                          f"lineages={len(self.type_set.phylogeny)}", flush=True)

                    # Update active type indices to include new types
                    self._active_type_indices = self.extant_type_indices

                    # Reset threshold for next Gillespie event
                    self.threshold_event_propensity = max(np.random.exponential(1.0), 1e-12)

                    # Stream event point as single-point epoch to keep GUI alive
                    if not hasattr(self, '_event_count'):
                        self._event_count = 0
                    self._event_count += 1

                    # Throttle logging (print every 10 events)
                    verbose = (self._event_count == 1 or self._event_count % 10 == 0)

                    if verbose:
                        print(f"[Community.run]   Streaming event point (event #{self._event_count})", flush=True)

                    # Build single-point epoch at t_event with ACTIVE types only
                    N_epoch_event = self.N[self._active_type_indices].reshape(-1, 1)  # (num_active, 1)
                    R_epoch_event = self.R.reshape(-1, 1)  # (num_resources, 1)

                    # Get active lineageIDs corresponding to N_epoch_event rows
                    active_lineageIDs = [self.type_set.lineageIDs[i] for i in self._active_type_indices]

                    if progress_callback is not None:
                        callback_data = {
                            't_epoch': np.array([t_event]),
                            'biomass_epoch': np.array([np.sum(N_epoch_event)]),
                            'N_epoch': N_epoch_event,  # Shape: (num_active_types, 1)
                            'R_epoch': R_epoch_event,
                            'lineageIDs_epoch': active_lineageIDs,  # NEW: lineageIDs for N_epoch rows
                            't_current': self.t,
                        ...
                        try:
                            ...
                            if verbose:
                                print(f"[Community.run]   Event point streamed: t={t_event:.3e}, "
                                      f"active_types={len(active_lineageIDs)}, total_types={self.type_set.num_types}",
                                      flush=True)
                        except Exception as e:
                            print(f"[Community.run]   ERROR: callback failed: {e}", flush=True)

                    # Continue to next epoch (skip fallback and normal time series update)
                    continue
                else:
                    # Event fired but t_events is empty - this shouldn't happen
                    print(f"[Community.run]   WARNING: Event fired but t_events is empty!", flush=True)

            # CRITICAL: Check if solve_ivp failed to advance time
            # MODIFIED: Only use fallback if solver failed WITHOUT an event
            if (len(sol.t) < 2 or sol.t[-1] <= self.t) and sol.status != 1:
                # FALLBACK STRATEGY: Guarantee 2-point integration with explicit t_eval
                print(f"[Community.run] WARNING: solve_ivp returned insufficient points. "
                      f"len(sol.t)={len(sol.t)}, sol.t[-1]={sol.t[-1]:.3e}, self.t={self.t:.3e}",
                      flush=True)

                # Calculate fallback step size
                dt_fallback = min(dt / 10.0, max_time_step / 12.0)
                t_span_fallback = (self.t, self.t + dt_fallback)

                # Ensure end > start
                if t_span_fallback[1] <= t_span_fallback[0]:
                    error_msg = (f"[Community.run] FATAL: Cannot create valid t_span. "
                                 f"self.t={self.t:.3e}, dt_fallback={dt_fallback:.3e}")
                    print(error_msg, flush=True)
                    raise RuntimeError(error_msg)

                print(f"[Community.run] Fallback: t_span={t_span_fallback}, events=None", flush=True)

                sol_fallback = scipy.integrate.solve_ivp(
                    self.dynamics,
                    y0=init_cond,
                    args=params,
                    t_span=t_span_fallback,
                    t_eval=None,  # Adaptive stepping
                    events=None,  # DISABLE events to force advancement
                    method=_integration_method,
                    max_step=dt_fallback / 2.0)  # Force small steps

                if len(sol_fallback.t) >= 2 and sol_fallback.t[-1] > self.t:
                    print(f"[Community.run] Fallback succeeded: len={len(sol_fallback.t)}, "
                          f"t=[{sol_fallback.t[0]:.3e}, {sol_fallback.t[-1]:.3e}]", flush=True)
                    sol = sol_fallback  # Use fallback solution
                else:
                    # Fallback also failed - this is truly fatal
                    error_msg = (f"[Community.run] FATAL: Fallback integration failed. "
                                 f"Primary: len(sol.t)={len(sol.t)}, sol.t={sol.t}. "
                                 f"Fallback: len={len(sol_fallback.t)}, t={sol_fallback.t}, "
                                 f"status={sol_fallback.status}, message={sol_fallback.message}")
                    print(error_msg, flush=True)
                    raise RuntimeError(error_msg)

            # Store old time for verification
            t_before_update = self.t
            ...
            if self.t <= t_before_update:
                error_msg = (f"[Community.run] FATAL: Time did not advance after adding sol.t[1:]. "
                             f"t_before={t_before_update:.3e}, t_after={self.t:.3e}, "
                             f"sol.t[-1]={sol.t[-1]:.3e}")
                print(error_msg, flush=True)
                raise RuntimeError(error_msg)

            print(f"[Community.run] Time advanced: {t_before_update:.3e} -> {self.t:.3e}", flush=True)

            #------------------------------
            # Send progress data to callback if provided (for GUI integration):
            #------------------------------
            if progress_callback is not None:
                # Send ACTIVE types only with corresponding lineageIDs
                N_epoch_active = N_epoch[self._active_type_indices, 1:]  # (num_active, num_times)
                active_lineageIDs = [self.type_set.lineageIDs[i] for i in self._active_type_indices]

                callback_data = {
                    't_epoch': sol.t[1:],                          # Send only NEW points (skip initial)
                    'biomass_epoch': np.sum(N_epoch_active, axis=0),  # Sum over active types, skip initial
                    'N_epoch': N_epoch_active,                     # Shape: (num_active_types, num_times)
                    'R_epoch': R_epoch[:, 1:],                     # Skip initial point
                    'lineageIDs_epoch': active_lineageIDs,         # NEW: lineageIDs for N_epoch rows
                    't_current': self.t,                           # Updated time (after sol.t[1:] added)
                    'epoch_completed': True                        # Flag indicating epoch end
                }
                ...
                if self._callback_count % 20 == 0:
                    print(f"[Community.run] Callback #{self._callback_count}: "
                          f"t_epoch len={len(sol.t)-1}, active_types={len(active_lineageIDs)}",
                          flush=True)
    ...
    def event_lineage_establishment(self, t, variables, *args):
        # GUARD: Prevent event from triggering at epoch start (t=0)
        # This prevents SciPy's root finder from detecting spurious zero-crossing at initial condition
        if hasattr(self, '_epoch_t0') and t <= self._epoch_t0 + 1e-12:
            # DIAGNOSTIC: Log when guard activates (only once per epoch)
            if not hasattr(self, '_guard_logged') or not self._guard_logged:
                self._guard_logged = True
                print(f"[Event Guard] Activated at t={t:.15e}, epoch_t0={self._epoch_t0:.15e}, "
                      f"delta={t - self._epoch_t0:.3e}", flush=True)
            return 1.0  # Always positive at epoch start, event cannot trigger

        # Reset guard log flag for next epoch
        if hasattr(self, '_guard_logged') and self._guard_logged:
    ...
    def handle_lineage_establishment(self):
        # DIAGNOSTIC LOGGING
        print(f"[Community] LINEAGE ESTABLISHMENT EVENT at t={self.t:.6e}", flush=True)
        ...
        print(f"[Community]   Propensities: mutation={total_mutation_propensity:.6e}, "
              f"segregation={total_segregation_propensity:.6e}, transfer={total_transfer_propensity:.6e}",
              flush=True)

        #----------------------------------
        r = np.random.uniform(0, total_establishment_propensity)
        if(r < total_mutation_propensity and total_mutation_propensity > 0):
            print(f"[Community]   -> Handling MUTATION (r={r:.6e} < {total_mutation_propensity:.6e})", flush=True)
            ...
            print(f"[Community]   -> Handling SEGREGATION", flush=True)
            self.handle_segregant_establishment()
        elif(total_transfer_propensity > 0):
            print(f"[Community]   -> Handling TRANSFER", flush=True)
            self.handle_transconjugant_establishment()
        else:
            print(f"[Community]   -> WARNING: No event type selected! r={r:.6e}, total={total_establishment_propensity:.6e}", flush=True)
            pass

    def handle_mutant_establishment(self):
        # DIAGNOSTIC LOGGING
        print(f"[Community]     handle_mutant_establishment() called, num_types_before={self.type_set.num_types}", flush=True)
        ...
        if(self.convergent_lineages and mutant_type_id in self.type_set.typeIDs):
            ...
            print(f"[Community]     MUTATION COMPLETE (convergent): Added to existing type, total types={self.type_set.num_types}", flush=True)
        else:
            ...
            # Remove corresonding abundance from the parent type (abundance is moved from parent to mutant):
            self.set_type_abundance(type_index=parent_idx, abundance=max(self.get_type_abundance(parent_idx) - mutant_abundance, 1))
            print(f"[Community]     MUTATION COMPLETE (new type): total types={self.type_set.num_types}, "
                  f"new lineageID={self.type_set.lineageIDs[-1] if len(self.type_set.lineageIDs) > 0 else 'N/A'}", flush=True)
        #----------------------------------
